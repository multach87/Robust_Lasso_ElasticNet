...
This file walks through the steps used to simulate data in DISSERTATION. WRITE MORE
...
Finally, let's map the data-generating function over all data conditions and iterations. Note that I've set "eval = F" for the full data-generating chunk below and the following *saveRDS()* chunk. The data file resulting from the chunk below would be nearly 2gb and it's unnecessary to process that for the html walkthrough. This also means that the following chunks won't operate properly, but I've included them here to show what would follow and for the purposes of storing everything on GitHub. Those who are interested in seeing it in action can set the "eval" arguments below to "eval = T". 

Function notes: *pmap()* will apply the function in parentheses (**data_gen** here) over all rows in the pre-existing object specified before the pipe (**sim_repped.dt** here). *pmap()* specifically applies the given function using the values in each row of the specified object as the arguments in the function being mapped. Here, it is being used to run the data-generating function individually for the conditions specified in each row of **sim_repped.dt**.
...
For use with subsequent demos of the ensemble lasso/elastic net procedure, I will be saving 100 random datasets from this full dataset. I've also included the vector of 100 random indices and the vector-generating seed for those indices for transparency.

```{r save 100 random datasets , eval = T}
#set the seed
set.seed(501)
...
Many of the lasso/elastic net functions used in the combined model I will be demo'ing rely on matrices rather than data.tables. Let's also create a version of this data file in which the X's are in matrix format.

In words, the following chunk is specifying a new objects **testing100.mtx**, which will result from the testing100 iterations of the full dataset. More specifically, it will extract just those iterations, but will apply the *as.matrix()* function to the appropriate list elements of each data iteration. Even more specifically, it applies as.matrix to elements 5 and 6 of each data iteration, which correspond with the X and Y data.tables, respectively.

```{r testing100 X matrices}
...
Let's also extract and process a single dataset, save it, and then do the same for a matrix-ized version as we did above.

```{r extract and save single dataset and mtx version}
# set random seed
set.seed(501)
...
saveRDS(testing1.mtx , "/Users/Matt/Desktop/GitHub_Dissertation_Scripts/Robust_Lasso_ElasticNet/Datasets/Testing1_Xmtx.RData")
```
...
saveRDS(testingapplied ,
        "/Users/Matt/Desktop/GitHub_Dissertation_Scripts/Robust_Lasso_ElasticNet/Datasets/TestingApplied.RData")
...
saveRDS(testingapplied.mtx ,
        "/Users/Matt/Desktop/GitHub_Dissertation_Scripts/Robust_Lasso_ElasticNet/Datasets/TestingApplied_Xmtx.RData")
