...
#colnames(sim_repped.dt) <- c("n" , "p" , "eta_x" , "eta_y" , "g" , "h" , 
#                                    "seed")
```
...
#purrr::map(seq_len(500) , ~sim_structure.dt[i , (1:6)])
```

Finally, generate a seed for each iteration. Let's also get rid of the blank column from initializing the data.table.
...
data_gen <- function(n , p , eta_x , eta_y , g , h , seed) {      
...
```{r save 10 random datasets , eval = F}
#set the seed
set.seed(501)
...
saveRDS(full.data[data.10] , "/FILE/PATH/Testing100.RData")
```

Finally, let's extract and process a single dataset. Since I'm going to be creating demos for use with applied datasets, let's make this is as close to a real dataset as possible. This mainly means I'm going to strip away the metadata I added for simulation purposes.

```{r extract single applied dataset , eval = F}
#set random seed
set.seed(501)
...
```{r strip metadata , eval = F}
#extract just our X and Y objects
testing1 <- list(Y_train = testing1[[1]]["Y_train"] , 
                 Y_hold = testing1[[1]]["Y_hold"] , 
                 X_train = testing1[[1]]["X_train"] , 
                 X_hold = testing1[[1]]["X_hold"])
```

And now we can save that resulting file.

```{r save single applied dataset , eval = F}
#save applied dataset
saveRDS(testing1 , "/FILE/PATH/TestingApplied.RData")
```