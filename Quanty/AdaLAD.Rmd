...
The function in the code chunk below applies the adaptive LAD lasso to specified data for predictors and a single response. X's should be in some tabular format (matrix, dataframe, data.table). Y can be in a similar format, or just a vector, but it must contain a number of elements equal to the number of rows of "xtrain" if given in a vector. Likewise, if your desired "ytrain" is in tabular format, the number of rows should equal number of rows of "xtrain." Currently, the function only works for a single response Y and is not intended for use with multiple simultaneous response variables.

There are separate arguments for the training predictors/response vs. the holdout predictors/response. The related functions and metrics (such as holdout prediction error) will only run if data is supplied to both of the holdout data arguments, "xhold" and "yhold."

```{r ladlasso model application function , echo = T}

# adaptive LAD lasso application function
ladlasso.sim.fnct <- function(xtrain , ytrain , 
                              xhold , yhold) {
       ...
       ridge.model <- cv.glmnet(x = xtrain , y = ytrain , 
       ...
       # using holdout data, if applicable, and depending on
       # # inclusion of intercept
       if(!is.null(xhold) & !is.null(yhold)) {
               # store n
               n <- nrow(xhold)
               
               # calculate predicted values
               y.pred <- xhold %*% ladlasso.coefs[-1] +
                               ladlasso.coefs[1]
...
Initially, this function was developed to give users the option of including or excluding a model intercept. However, the *hqreg* family of functions inherently include an intercept in model estimation. Consequently, excluding the intercept might create issues with proper interpretation of the results, particularly if prediction accuracy is of interest.

Now let's try running the model on our demo.data. Note that we're mapping/applying the model using the *safely()* function. This allows our model application function to continue mapping after running into an error and provide more info about errors that arise.

```{r run adalad function}
ladlasso.test <- demo.data[1] %>%   
       map(safely(ladlasso.sim.fnct) , 
           xtrain = X_train , ytrain = Y_train , 
           xhold = X_hold , yhold = Y_hold)
```