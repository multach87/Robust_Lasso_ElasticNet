...
This file demonstrates use of a custom function for indexing and splitting a full dataset for _k_-fold cross-validation. Use this procedure if you intend to utilize out-of-sample prediction error in some fashion for your application. 

If out-of-sample prediction error is _not_ relevant for your application and you would just like a resampling procedure for improved model results, see 
 ...
 * 'list' : this argument tells the function if you want the resulting *k*-fold indexed data to be put into list format, with each list element corresponding with one fold *k*
   ...
   * If set to *"traintest"* (in quotes!), the resulting list will instead contain six elements:
     ...
     * "Seed" : The random-generating seed for the dataset. This is primarily for convenience with the multi-split wrapper function in <span style = "color:green">`` `r "02B_KFold_Multi"` ``</span>
...
The .Rmd file contains deprecated versions of the subsetting function at the end of the report for those who are interested. 
...
## Deprecated Versions of **<span style = "color:blue">`` `r "kfold_subsetter()"` ``</span>**
